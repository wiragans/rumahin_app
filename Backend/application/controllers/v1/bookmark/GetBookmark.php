...
class GetBookmark extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            //
            $getBookmarkData = $this->db->query("SELECT * FROM bookmark_lists WHERE BINARY username = ? AND isDeleted = 0", array($getFixUsername));
            $doGetBookmarkData = $getBookmarkData->result_array();

            $arrayBookmarkData = array();

            foreach($doGetBookmarkData as $rowBookmarkData)
            {
                $listingID = intval($rowBookmarkData['id']);
                $getKatalogUUID = $rowBookmarkData['katalogUUID'];

                // VERIFIKASI KATALOG SUDAH DIHAPUS APA BELUM OLEH OWNERNYA
                $verifikasiKatalog = $this->db->query("SELECT * FROM katalog_data WHERE BINARY katalogUUID = ? AND status = 1", array($getKatalogUUID));
                $doVerifikasiKatalog = $verifikasiKatalog->result_array();

                if(count($doVerifikasiKatalog) > 0)
                {
                    foreach($doVerifikasiKatalog as $rowKatalogData)
                    {
                        $katalogName = htmlentities(trim($rowKatalogData['katalogName']), ENT_QUOTES, 'UTF-8');
                        $alamat = htmlentities(trim($rowKatalogData['alamat']), ENT_QUOTES, 'UTF-8');
                        $luasBangunan = floatval($rowKatalogData['luasBangunan']);
                        $luasTanah = floatval($rowKatalogData['luasTanah']);
                        $hargaInt = intval($rowKatalogData['harga']);
                        $hargaStr = "Rp. " . number_format($hargaInt,2,',','.');
                        $katalogShortDesc = substr($rowKatalogData['katalogDesc'], 0, 100) . " ...."; // limit 100 karakter

                        $getKatalogImageData = $this->db->query("SELECT * FROM katalog_images_data WHERE BINARY katalogUUID = ? ORDER BY id ASC LIMIT 1", array($getKatalogUUID));
                        $doGetKatalogImagesData = $getKatalogImageData->result_array();

                        foreach($doGetKatalogImagesData as $rowKatalogImagesData)
                        {
                            $imagesUrlnya = $this->config->item('baseUrlKatalogImages') . $rowKatalogImagesData['imagesUrl'];
                        }

                        $arrayBookmarkData2 = [
                            'bookmarkListingID'=>$listingID,
                            'katalogUUID'=>$getKatalogUUID,
                            'katalogName'=>$katalogName,
                            'alamat'=>$alamat,
                            'katalogShortDesc'=>$katalogShortDesc,
                            'luasBangunan'=>$luasBangunan,
                            'luasTanah'=>$luasTanah,
                            'hargaInt'=>$hargaInt,
                            'hargaStr'=>$hargaStr,
                            'katalogImageUrl'=>$imagesUrlnya
                        ];

                        array_push($arrayBookmarkData, $arrayBookmarkData2);
                    }
                }
            }
            ...
            echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$arrayBookmarkData), 200, 'application/json; charset=UTF-8');
