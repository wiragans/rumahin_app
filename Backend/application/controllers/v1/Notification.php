...
class Notification extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            // get pengumuman notification lists
            $pengumumanLists = $this->db->query("SELECT * FROM pengumuman WHERE isShow = 1 ORDER BY announcedAtTimestamp DESC");
            $resultPengumumanLists = $pengumumanLists->result_array();

            $arrayPengumuman = array();

            foreach($resultPengumumanLists as $rowPengumumanLists)
            {
                $getIdPeng = intval($rowPengumumanLists['id']);
                $pengumumanUUID = $rowPengumumanLists['pengumumanUUID'];
                $titlePengumuman = $rowPengumumanLists['titlePengumuman'];
                $contentPengumuman = $rowPengumumanLists['contentPengumuman'];
                $announcedAtTimestamp = intval($rowPengumumanLists['announcedAtTimestamp']);
                $realDate = date("Y-m-d H:i:s", $announcedAtTimestamp) . " WIB";

                $arrayPengumuman2 = [
                    'id'=>$getIdPeng,
                    'pengumumanUUID'=>$pengumumanUUID,
                    'titlePengumuman'=>$titlePengumuman,
                    'contentPengumuman'=>$contentPengumuman,
                    'announcedAtTimestamp'=>$announcedAtTimestamp,
                    'realDate'=>$realDate
                ];

                array_push($arrayPengumuman, $arrayPengumuman2);
            }

            sleep(1);

            echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$arrayPengumuman), 200, 'application/json; charset=UTF-8');
