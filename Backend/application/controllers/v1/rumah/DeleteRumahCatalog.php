...
class DeleteRumahCatalog extends REST_Controller
{
    ...
    public function index_post()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            //
            $getKatalogUUID = trim($rawData['katalogUUID']);

            if(empty(trim($getKatalogUUID)))
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Katalog UUID Kosong!', 'data'=>[]), 200, 'application/json; charset=UTF-8');

                exit();
            }

            $validasiKatalogUUID = $this->db->query("SELECT * FROM katalog_data WHERE BINARY username = ? AND BINARY katalogUUID = ? AND status <> 4", array($getFixUsername, $getKatalogUUID));
            $validasiKatalogUUIDResult = $validasiKatalogUUID->result_array();

            if(count($validasiKatalogUUIDResult) <= 0)
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Katalog UUID tidak valid atau tidak dapat ditemukan!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
            }

            if(count($validasiKatalogUUIDResult) > 0)
            {
                //echo json_encode($validasiKatalogUUIDResult);
                //exit();

                //UPDATE KE STATUS 4 (DELETED)
                $updateStatusDeleted = $this->db->query("UPDATE katalog_data SET status = 4 WHERE BINARY katalogUUID = ?", array($getKatalogUUID));

                $updateStatusDeleted = true;

                if($updateStatusDeleted)
                {
                    // ** SIMPAN HISTORY DELETE - CEGAH FRAUD DLL ** //
                    $execution_timestamp = time();
                    $backup_main_content_array = json_encode($validasiKatalogUUIDResult);
                    $actionRumah = "delete";

                    $backup_misc_content_array = "";
                    $getMiscContent = $this->db->query("SELECT * FROM misc_katalog_spec WHERE BINARY katalogUUID = ?", array($getKatalogUUID));
                    $resultGetMiscContent = $getMiscContent->result_array();

                    if(count($resultGetMiscContent) > 0)
                    {
                        $backup_misc_content_array = json_encode($resultGetMiscContent);
                    }

                    $backup_images_content_array = "";
                    $getImagesContent = $this->db->query("SELECT * FROM katalog_images_data WHERE BINARY katalogUUID = ?", array($getKatalogUUID));
                    $resultGetImagesContent = $getImagesContent->result_array();

                    if(count($resultGetImagesContent) > 0)
                    {
                        $backup_images_content_array = json_encode($resultGetImagesContent);
                    }

                    $backup_video_content_array = "";
                    $getVideoContent = $this->db->query("SELECT * FROM katalog_video_data WHERE BINARY katalogUUID = ?", array($getKatalogUUID));
                    $resultGetVideoContent = $getVideoContent->result_array();

                    if(count($resultGetVideoContent) > 0)
                    {
                        $backup_video_content_array = json_encode($resultGetVideoContent);
                    }

                    $backup_ar_data_content_array = "";
                    $getARDataContent = $this->db->query("SELECT * FROM ar_data WHERE BINARY katalogUUID = ?", array($getKatalogUUID));
                    $resultGetARDataContent = $getARDataContent->result_array();

                    if(count($resultGetARDataContent) > 0)
                    {
                        $backup_ar_data_content_array = json_encode($resultGetARDataContent);
                    }

                    $insertDeleteHistory = $this->db->query("INSERT INTO katalog_edit_delete_history(username, backup_main_content_array, backup_misc_content_array, backup_images_content_array, backup_video_content_array, backup_ar_data_content_array, execution_timestamp, action) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", array($getFixUsername, $backup_main_content_array, $backup_misc_content_array, $backup_images_content_array, $backup_video_content_array, $backup_ar_data_content_array, $execution_timestamp, $actionRumah));

                    // ******************************************************************************************** //

                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'Katalog Anda berhasil dihapus', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                }

                else
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Terjadi kesalahan saat menghapus katalog, silakan coba lagi beberapa saat!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                }
            }
        }
