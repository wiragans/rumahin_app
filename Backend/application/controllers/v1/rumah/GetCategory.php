...
class GetCategory extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            // get category rumah tinggal / jenisnya
            $getCategory = $this->db->query("SELECT * FROM rumah_data");
            $resultGetCategory = $getCategory->result_array();

            $arrayRumahCat = array();

            foreach($resultGetCategory as $rowCategory)
            {
                $getIdCat = intval($rowCategory['id']);
                $kodeTipeRumah = $rowCategory['kodeTipeRumah'];
                $tipePropertiRumahInString = $rowCategory['tipePropertiRumahInString'];
                $shortName = $rowCategory['shortName'];

                $arrayRumahCat2 = [
                    'id'=>$getIdCat,
                    'kodeTipeRumah'=>$kodeTipeRumah,
                    'tipePropertiRumahInString'=>$tipePropertiRumahInString,
                    'shortName'=>$shortName
                ];

                array_push($arrayRumahCat, $arrayRumahCat2);
            }

            echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$arrayRumahCat), 200, 'application/json; charset=UTF-8');
