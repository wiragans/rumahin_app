...
class EditRumahCatalog extends REST_Controller
{
    ...
    public function index_post()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            //
                ...
                $katalogUUIDGet = trim($this->input->post('katalogUUID', TRUE));
            ...
            // VALIDATE KATALOG UUID

            $validateKatalogUUID = $this->db->query("SELECT * FROM katalog_data WHERE BINARY katalogUUID = ? AND BINARY username = ? AND status = 1", array($katalogUUIDGet, $getFixUsername));
            $resultValidateKatalogUUID = $validateKatalogUUID->result_array();

            if(count($resultValidateKatalogUUID) <= 0)
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Katalog rumah ini tidak valid atau tidak dapat ditemukan di sistem kami!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
            ...
            [
                ...
                'errors' => array(
                    ...
                    'greater_than'=> json_encode(array('message'=>'Isikan rumpang jumlah tingkat/lantai dengan angka 0 untuk mengosongkan'))
                ),
            ],
            [
                'field'=>'tahunDibuat',
                ...
                if(count($resultValJenisRumah) > 0)
                {
                    ...
                    {           
                            ...
                            else
                            {
                                ...
                                $deleteImages = $this->db->query("DELETE FROM katalog_images_data WHERE BINARY katalogUUID = ?", array($katalogUUIDGet));

                                $insertImages = $this->db->query("INSERT INTO katalog_images_data(imagesUrl, imagesFormat, fileSize, katalogUUID) VALUES(?, ?, ?, ?)", array($imagesPath, $dataInfo['file_ext'], strval($dataInfo['file_size']), $katalogUUIDGet));
                    ...
                    $totalViewed = 0;
                    $insertKatalog = $this->db->query("UPDATE katalog_data SET katalogName = ?, katalogDesc = ?, luasTanah = ?, luasBangunan = ?, jumlahKamarMandi = ?, jumlahKamarTidur = ?, jumlahRuangTamu = ?, jumlahGarasi = ?, jumlahRuangKeluarga = ?, jumlahRuangMakan = ?, jumlahGudang = ?, jumlahSerambi = ?, jumlahTingkat = ?, harga = ?, developerName = ?, contactNumber = ?, emailDeveloper = ?, alamat = ?, sertifikat = ?, isDisewakan = ?, isSecond = ?, kodeTipeRumah = ?, modeSewa = ?, tahunDibuat = ?, useAR = ?, sertifikat_code = ?, provinsi_id = ?, kabupaten_id = ?, kecamatan_id = ?, desa_id = ?, jumlahDapur = ? WHERE BINARY katalogUUID = ?", array($katalogName, $katalogDesc, $luasTanah, $luasBangunan, $jumlahKamarMandi, $jumlahKamarTidur, $jumlahRuangTamu, $jumlahGarasi, $jumlahRuangKeluarga, $jumlahRuangMakan, $jumlahGudang, $jumlahSerambi, $jumlahTingkat, $harga, $developerName, $contactNumber, $developerEmail, $alamat, $sertifikat, $isDisewakan, $isSecond, $kodeTipeRumah, $modeSewa, $tahunDibuat, $useAR, $sertifikatCodeGet, $provinsiIDGet, $kabupatenIDGet, $kecamatanIDGet, $desaIDGet, $jumlahDapur, $katalogUUIDGet));

                    $insertMiscData = $this->db->query("UPDATE misc_katalog_spec SET tipePropertiRumah = ?, conditionMeasurement = ?, perlengkapanPerabotan = ?, dayaListrik = ? WHERE BINARY katalogUUID = ?", array($kodeTipeRumah, $conditionMeasurement, $perlengkapanPerabotan, $dayaListrik, $katalogUUIDGet));

                    //

                    $deleteVideo = $this->db->query("DELETE FROM katalog_video_data WHERE BINARY katalogUUID = ?", array($katalogUUIDGet));

                    $insertVideo = $this->db->query("INSERT INTO katalog_video_data(videoUrl, katalogUUID) VALUES(?, ?)", array($fixedYouTubeVideoUrl, $katalogUUIDGet));
                    ...
                    if($useAR == 1)
                    {
                        ...
                        $deleteARData = $this->db->query("DELETE FROM ar_data WHERE BINARY katalogUUID = ?", array($katalogUUIDGet));

                        $objectFileUUID = $this->guidv4();
                        $insertARData = $this->db->query("INSERT INTO ar_data(objectFileURL, objectFileDiffuseTextureURL, updateTimestamp, markerBase64Format, markerUrl, katalogUUID) VALUES(?, ?, ?, ?, ?, ?)", array($fbxFileLink, $fbxLinkDiffuseTexture, $updateTimestamp, $gambarMarkerBase64, $linkGambarMarker, $katalogUUIDGet));
                    }
                    ...
                    $stemmingPerWordText = $this->stemming($tokenizingPerWordText, $katalogUUIDGet); // DECODE ARRAY DAN LAKUKAN STEMMING PENCARIAN KATA DASARNYA, DAN MASUKKAN DI DATABASE TABLE TOKEN

                    //

                    // JSON OUTPUTNYA
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'Edit Katalog Rumah Sukses', 'data'=>[
                        'katalogImagesArray'=>$arrayFileInfoImages,
                        'katalogUUID'=>$katalogUUIDGet,
                        'clickTrackingParams'=>[
                            'routeTo'=>'/katalog/view/' . $katalogUUIDGet,
            ...
            else
            {
                ...
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'code'=>'EDIT_KATALOG_ERROR', 'status'=>false, 'message'=>'Permintaan tidak diterima, periksa kembali data yang diinputkan di atas!', 
                    'data'=>[
                    'errors'=>$arrayError
                    ]), 200, 'application/json; charset=UTF-8');
    ...
    public function stemming($stemmingData, $katalogUUIDnya)
	{
		...
		for($cobaUlangi = 0; $cobaUlangi < sizeof($getStemmingData['tokenizingResult']); $cobaUlangi++)
		{
            ...
            $deleteToken = "DELETE FROM stem_token_data WHERE BINARY katalogUUID = ?";
            $doDeleteToken = $this->db->query($deleteToken, array($katalogUUIDnya));
