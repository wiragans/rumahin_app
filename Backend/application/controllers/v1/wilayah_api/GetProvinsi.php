...
class GetProvinsi extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $getProvinsi = $this->db->query("SELECT * FROM wilayah_provinsi");
            $resultGetProvinsi = $getProvinsi->result_array();

            $provinsiDataArray = array();

            foreach($resultGetProvinsi as $rowDataProvinsi)
            {
                $idProvinsi = intval($rowDataProvinsi['id']);
                $namaProvinsi = $rowDataProvinsi['nama'];
                ...
                array_push($provinsiDataArray, $provinsiDataArray2);
            }

            $keysSortArray = array_column($provinsiDataArray, 'nama');
            array_multisort($keysSortArray, SORT_ASC, $provinsiDataArray);

            echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$provinsiDataArray), 200, 'application/json; charset=UTF-8');
