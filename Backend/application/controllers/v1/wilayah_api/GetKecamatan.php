...
class GetKecamatan extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $kabupatenID = strval($this->input->get('kabupaten_id', TRUE));

            $getKecamatan = $this->db->query("SELECT * FROM wilayah_kecamatan WHERE kabupaten_id = ?", array($kabupatenID));
            $resultGetKecamatan = $getKecamatan->result_array();

            $dataArray = array();

            foreach($resultGetKecamatan as $rowDataKecamatan)
            {
                $idKecamatan = intval($rowDataKecamatan['id']);
                $kabupatenIDGet = intval($rowDataKecamatan['kabupaten_id']);
                $namaKecamatan = $rowDataKecamatan['nama'];

                $dataArray2 = [
                    'id'=>$idKecamatan,
                    'kabupaten_id'=>$kabupatenIDGet,
                    'nama'=>$namaKecamatan
                ];
