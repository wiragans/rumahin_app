...
class GetKabKota extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $getKabKota = $this->db->query("SELECT * FROM wilayah_kabupaten WHERE provinsi_id = ?", array($this->input->get('provinsi_id', TRUE)));
            $resultGetKabKota = $getKabKota->result_array();

            $dataArray = array();

            foreach($resultGetKabKota as $rowDataKabKota)
            {
                $idKabKota = intval($rowDataKabKota['id']);
                $provinsi_id = intval($rowDataKabKota['provinsi_id']);
                $namaKabKota = $rowDataKabKota['nama'];
