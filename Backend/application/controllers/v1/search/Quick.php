...
class Quick extends REST_Controller
{
    ...
    public function index_post()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            if($statusUser == 2)
            {
                ...
                $usePreferences = boolval($rawData['usePreferences']);
                $preferencesContext = $rawData['context'];

                //var_dump($preferencesContext);
                //exit();
                ...
                if($getLimitOffset > 30)
                {
                    ...
                    foreach($preferencesContext as $rowServicesContext)
                    {
                        if($rowServicesContext['service'] == "PRICE_PREFERENCES")
                        {
                            $servicesParams = $rowServicesContext['params'];

                            foreach($servicesParams as $rowServiceParams)
                            {
                                if($rowServiceParams['key'] == "MINIMUM_PRICE")
                                {
                                    $minimumPrice = intval($rowServiceParams['value']);
                                }

                                if($rowServiceParams['key'] == "MAXIMUM_PRICE")
                                {
                                    $maximumPrice = intval($rowServiceParams['value']);
                                }
                            }
                        }

                        if($rowServicesContext['service'] == "CATALOG_CONDITIONS_PREFERENCES")
                        {
                            $servicesParams = $rowServicesContext['params'];

                            foreach($servicesParams as $rowServiceParams)
                            {
                                if($rowServiceParams['key'] == "BUY_OR_RENT")
                                {
                                    $buyOrRent = intval($rowServiceParams['value']);
                                }

                                if($rowServiceParams['key'] == "NEW_OR_SECOND")
                                {
                                    $newOrSecond = intval($rowServiceParams['value']);
                                }
                            }
                        }
            ...
            $nextUrl = base_url() . 'v1/search/quick?page=' . $nextPage . '&limit=' . $getLimitOffset;

            // ARRAY
            if($usePreferences == true)
            {
                $getDataKatalog = $this->db->query("SELECT * FROM katalog_data WHERE (harga <= ? AND harga >= ?) AND (isDisewakan = ?) AND (isSecond = ?) AND (BINARY kodeTipeRumah = ?) AND (provinsi_id = ? OR kabupaten_id = ? OR kecamatan_id = ? OR desa_id = ?) AND status = 1 LIMIT ?, ?", array($maximumPrice, $minimumPrice, $buyOrRent, $newOrSecond, $catatalogTypeValue, $provinsiIDValue, $kabupatenIDValue, $kecamatanIDValue, $desaIDValue, intval($startFrom), intval($getLimitOffset)));
            }

            if($usePreferences == false)
            {
                $getDataKatalog = $this->db->query("SELECT * FROM katalog_data WHERE (harga <= ? AND harga >= ?) AND (isDisewakan = ?) AND (isSecond = ?) AND status = 1 ORDER BY id DESC LIMIT ?, ?", array($maximumPrice, $minimumPrice, $buyOrRent, $newOrSecond, intval($startFrom), intval($getLimitOffset)));
            }

            //$getDataKatalog = $this->db->query("SELECT * FROM katalog_data LIMIT ?, ?", array(intval($startFrom), intval($getLimitOffset)));
            $resultGetDataKatalog = $getDataKatalog->result_array();

            //var_dump($resultGetDataKatalog); exit();

            //echo $provinsiIDValue;

            //exit();

            if(count($resultGetDataKatalog) <= 0)
            {
                //sleep(1);
            ...
            if(count($resultGetDataKatalog) > 0)
            {
                ...
                foreach($resultGetDataKatalog as $rowDataKatalog)
                {
                    $katalogID = intval($rowDataKatalog['id']);
                    $katalogUUIDnya = $rowDataKatalog['katalogUUID'];
                    $katalogName = htmlentities(trim($rowDataKatalog['katalogName']), ENT_QUOTES, 'UTF-8');

                    $luasBanguannya = floatval($rowDataKatalog['luasBangunan']);
                    $luasTanahnya = floatval($rowDataKatalog['luasTanah']);

                    $alamatnya = $rowDataKatalog['alamat'];

                    $priceInt = intval($rowDataKatalog['harga']);
                    ...
                    array_push($arrayDataFix, $arrayDataFix2);
                }

                //sleep(1);

                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$arrayDataFix, 'nextPage'=>$nextUrl), 200, 'application/json; charset=UTF-8');
