...
class NewKatalogMore extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            if($useFiltering == 1)
            {
                ...
                {
                    $getNewKatalog = $this->db->query("SELECT * FROM katalog_data WHERE status = 1 AND provinsi_id = ? AND kabupaten_id = ? AND kecamatan_id = ? AND desa_id = ? AND BINARY kodeTipeRumah = ? ORDER BY tayangTimestamp DESC LIMIT ?, ?", array($provinsiIDGet, $kabupatenIDGet, $kecamatanIDGet, $desaIDGet, $catalogTypeGet, intval($startFrom), intval($getLimitOffset)));
                    $resultGetNewKatalog = $getNewKatalog->result_array();
                }

                if($regionPreferencesSuccess == true && $catalogTypePreferencesSuccess == false)
                {
                    $getNewKatalog = $this->db->query("SELECT * FROM katalog_data WHERE status = 1 AND provinsi_id = ? AND kabupaten_id = ? AND kecamatan_id = ? AND desa_id = ? ORDER BY tayangTimestamp DESC LIMIT ?, ?", array($provinsiIDGet, $kabupatenIDGet, $kecamatanIDGet, $desaIDGet, intval($startFrom), intval($getLimitOffset)));
                    $resultGetNewKatalog = $getNewKatalog->result_array();
                }

                if($regionPreferencesSuccess == false && $catalogTypePreferencesSuccess == true)
                {
                    $getNewKatalog = $this->db->query("SELECT * FROM katalog_data WHERE status = 1 AND BINARY kodeTipeRumah = ? ORDER BY tayangTimestamp DESC LIMIT ?, ?", array($catalogTypeGet, intval($startFrom), intval($getLimitOffset)));
                    $resultGetNewKatalog = $getNewKatalog->result_array();
                }
            }
            ...
            if($useFiltering != 1)
            {
                $getNewKatalog = $this->db->query("SELECT * FROM katalog_data WHERE status = 1 ORDER BY tayangTimestamp DESC LIMIT ?, ?", array(intval($startFrom), intval($getLimitOffset)));
                $resultGetNewKatalog = $getNewKatalog->result_array();
            }

            $nextPage = $getPageOffset + 1;
            $nextUrl = base_url() . 'v1/front/more/newKatalog?page=' . $nextPage . '&limit=' . $getLimitOffset;

            $arrayKatalogList = array();

            foreach($resultGetNewKatalog as $rowNewKatalog)
            {
                $getKatalogImages = $this->db->query("SELECT * FROM katalog_images_data WHERE BINARY katalogUUID = ? ORDER BY id ASC LIMIT 1", array($rowNewKatalog['katalogUUID']));
                ...
                $provinsiIDGetDB = strval($rowNewKatalog['provinsi_id']);
                $kabupatenIDGetDB = strval($rowNewKatalog['kabupaten_id']);
                $kecamatanIDGetDB = strval($rowNewKatalog['kecamatan_id']);
                $desaIDGetDB = strval($rowNewKatalog['desa_id']);
                ...
                $arrayKatalogList2 = [
                    'id'=>intval($rowNewKatalog['id']),
                    'katalogUUID'=>$rowNewKatalog['katalogUUID'],
                    'katalogName'=>htmlentities(trim($rowNewKatalog['katalogName']), ENT_QUOTES, 'UTF-8'),
                    'katalogDesc'=>$rowNewKatalog['katalogDesc'],
                    'fixedAlamat'=>$desannya . ', ' . $kecamatannya . ', ' . $kabupatennya . ', ' . $provinsinya,
                    'alamat'=>$rowNewKatalog['alamat'],
                    'totalViewed'=>intval($rowNewKatalog['totalViewed']),
                    'luasBangunan'=>floatval($rowNewKatalog['luasBangunan']),
                    'luasTanah'=>floatval($rowNewKatalog['luasTanah']),
                    'priceInt'=>intval($rowNewKatalog['harga']),
                    'priceStr'=>"Rp. " . number_format(intval($rowNewKatalog['harga']),2,',','.'),
                    'publishedAt'=>date("Y-m-d H:i:s", $rowNewKatalog['tayangTimestamp']) . ' WIB',
