...
class GetCatalogType extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $getCatalogTypeData = $this->db->query("SELECT * FROM catalog_type_user_pref WHERE BINARY username = ?", array($getFixUsername));
            $resultGetCatalogTypeData = $getCatalogTypeData->result_array();

            if($getCatalogTypeData)
            {
                if(count($resultGetCatalogTypeData) <= 0)
                {
                ...
                if(count($resultGetCatalogTypeData) > 0)
                {
                    foreach($resultGetCatalogTypeData as $rowCatalogTypePref)
                    {
                        $catalogType = $rowCatalogTypePref['catalog_type'];
                    }

                    if(!empty(trim($catalogType)))
                    {
                        $getCatalogTypeInfo = $this->db->query("SELECT * FROM rumah_data WHERE BINARY kodeTipeRumah = ?", array($catalogType));
                        $resultGetCatalogTypeInfo = $getCatalogTypeInfo->result_array();

                        foreach($resultGetCatalogTypeInfo as $rowCatalogTypeInfo)
                        {
                            $catalogNamaPanjang = $rowCatalogTypeInfo['tipePropertiRumahInString'];
                            $catalogNamaShort = $rowCatalogTypeInfo['shortName'];
                        }
                    }
                    ...
                    $resultJson = [
                        'catalogType'=>[
                            'code'=>$catalogType,
                            'namaPanjang'=>$catalogNamaPanjang,
                            'namaPendek'=>$catalogNamaShort
                        ]
                      ];
            ...
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'code'=>'FETCH_CATALOG_TYPE_PREF_ERROR', 'status'=>false, 'message'=>'Ada kesalahan, coba lagi!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
