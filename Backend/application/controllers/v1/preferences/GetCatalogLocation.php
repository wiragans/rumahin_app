...
class GetCatalogLocation extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $getCatalogLocationPref = $this->db->query("SELECT * FROM catalog_user_pref WHERE BINARY username = ?", array($getFixUsername));
            $resultGetCatalogLocationPref = $getCatalogLocationPref->result_array();

            if($getCatalogLocationPref)
            {
                if(count($resultGetCatalogLocationPref) <= 0)
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'( Belum diatur )', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultGetCatalogLocationPref) > 0)
                {
                    foreach($resultGetCatalogLocationPref as $rowCatalogPref)
                    {
                        $provinsi_id_get = $rowCatalogPref['provinsi_id'];

                        if(!empty(trim($provinsi_id_get)))
                        {
                            $getProvinsiIDInfo = $this->db->query("SELECT * FROM wilayah_provinsi WHERE id = ?", array($provinsi_id_get));
                            $resultGetProvinsiIDInfo = $getProvinsiIDInfo->result_array();

                            foreach($resultGetProvinsiIDInfo as $rowProvinsiInfo)
                            {
                                $provinsiNama = $rowProvinsiInfo['nama'];
                            }
                        }

                        $kabupaten_id_get = $rowCatalogPref['kabupaten_id'];

                        if(!empty(trim($kabupaten_id_get)))
                        {
                            $getKabupatenIDInfo = $this->db->query("SELECT * FROM wilayah_kabupaten WHERE id = ?", array($kabupaten_id_get));
                            $resultGetKabupatenIDInfo = $getKabupatenIDInfo->result_array();

                            foreach($resultGetKabupatenIDInfo as $rowKabupatenInfo)
                            {
                                $kabupatenNama = $rowKabupatenInfo['nama'];
                            }
                        }

                        $kecamatan_id_get = $rowCatalogPref['kecamatan_id'];

                        if(!empty(trim($kecamatan_id_get)))
                        {
                            $getKecamatanIDInfo = $this->db->query("SELECT * FROM wilayah_kecamatan WHERE id = ?", array($kecamatan_id_get));
                            $resultGetKecamatanIDInfo = $getKecamatanIDInfo->result_array();

                            foreach($resultGetKecamatanIDInfo as $rowKecamatanInfo)
                            {
                                $kecamatanNama = $rowKecamatanInfo['nama'];
                            }
                        }

                        $desa_id = $rowCatalogPref['desa_id'];

                        if(!empty(trim($desa_id)))
                        {
                            $getDesaIDInfo = $this->db->query("SELECT * FROM wilayah_desa WHERE id = ?", array($desa_id));
                            $resultGetDesaIDInfo = $getDesaIDInfo->result_array();

                            foreach($resultGetDesaIDInfo as $rowDesaInfo)
                            {
                                $desaNama = $rowDesaInfo['nama'];
                            }
                        }
                        ...
                        echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'SUCCESS', 'data'=>$resultJson), 200, 'application/json; charset=UTF-8');
            ...
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'code'=>'FETCH_CATALOG_PREF_ERROR', 'status'=>false, 'message'=>'Ada kesalahan, coba lagi!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
