...
class SetCatalogLocation extends REST_Controller
{
    ...
    public function index_post()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $getProvinsiID = trim($rawData['provinsiId']);
            $getKabupatenID = trim($rawData['kabupatenId']);
            $getKecamatanID = trim($rawData['kecamatanId']);
            $getDesaID = trim($rawData['desaId']);
            ...
            if(empty(trim($getProvinsiID)))
            {
                echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Data Provinsi harus diatur!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
            ...
            if(!empty(trim($getProvinsiID)))
            {
                $validasiProvinsiID = $this->db->query("SELECT * FROM wilayah_provinsi WHERE id = ?", array($getProvinsiID));
                $resultValidasiProvinsiID = $validasiProvinsiID->result_array();

                if(count($resultValidasiProvinsiID) <= 0)
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Data Provinsi tidak valid. Periksa kembali ya!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultValidasiProvinsiID) > 0)
                {
                    $fixedProvinsiID = $getProvinsiID;
                }
            }

            if(!empty(trim($getKabupatenID)))
            {
                $validasiKabupatenID = $this->db->query("SELECT * FROM wilayah_kabupaten WHERE id = ?", array($getKabupatenID));
                $resultValidasiKabupatenID = $validasiKabupatenID->result_array();

                if(count($resultValidasiKabupatenID) <= 0)
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Data Kabupaten tidak valid. Periksa kembali ya!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultValidasiKabupatenID) > 0)
                {
                    $fixedKabupatenID = $getKabupatenID;
                }
            }

            if(!empty(trim($getKecamatanID)))
            {
                $validasiKecamatanID = $this->db->query("SELECT * FROM wilayah_kecamatan WHERE id = ?", array($getKecamatanID));
                $resultValidasiKecamatanID = $validasiKecamatanID->result_array();

                if(count($resultValidasiKecamatanID) <= 0)
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Data Kecamatan tidak valid. Periksa kembali ya!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultValidasiKecamatanID) > 0)
                {
                    $fixedKecamatanID = $getKecamatanID;
                }
            }

            if(!empty(trim($getDesaID)))
            {
                $validasiDesaID = $this->db->query("SELECT * FROM wilayah_desa WHERE id = ?", array($getDesaID));
                $resultValidasiDesaID = $validasiDesaID->result_array();

                if(count($resultValidasiDesaID) <= 0)
                {
                    echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>false, 'message'=>'Data Desa tidak valid. Periksa kembali ya!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultValidasiDesaID) > 0)
                {
                    $fixedDesaID = $getDesaID;
                }
            }
            
            //

            $validasiCurrentData = $this->db->query("SELECT * FROM catalog_user_pref WHERE BINARY username = ?", array($getFixUsername));
            ...
            if($validasiCurrentData)
            {
                ...
                {
                    $insertQueryData = $this->db->query("INSERT INTO catalog_user_pref(username, provinsi_id, kabupaten_id, kecamatan_id, desa_id) VALUES(?, ?, ?, ?, ?)", array($getFixUsername, $fixedProvinsiID, $fixedKabupatenID, $fixedKecamatanID, $fixedDesaID));

                    if($insertQueryData)
                    {
                        echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'Berhasil mengatur setelan preferensi lokasi katalog!', 'data'=>[]), 200, 'application/json; charset=UTF-8');
                ...
                if(count($resultValidasiCurrentData) > 0)
                {
                    $updateQueryData = $this->db->query("UPDATE catalog_user_pref SET provinsi_id = ?, kabupaten_id = ?, kecamatan_id = ?, desa_id = ? WHERE BINARY username = ?", array($fixedProvinsiID, $fixedKabupatenID, $fixedKecamatanID, $fixedDesaID, $getFixUsername));
