...
class SertifikatType extends REST_Controller
{
	...
	public function index_get()
	{
        ...
        if(count($resultValidasiAuth) > 0)
        {
            ...
            $sertifikatTypeGet = $this->db->query("SELECT * FROM sertifikat_lists");
            $resultSertifikatTypeGet = $sertifikatTypeGet->result_array();

            $arrayDatanya = array();

            foreach($resultSertifikatTypeGet as $rowResultSertifikatType)
            {
                $idnya = intval($rowResultSertifikatType['id']);
                $sertifikatCodeGet = $rowResultSertifikatType['sertifikat_code'];
                $sertifikatNameGet = $rowResultSertifikatType['sertifikat_name'];
                $needManualInput = intval($rowResultSertifikatType['need_manual_input']);

                $arrayDatanya2 = [
                    'id'=>$idnya,
                    'sertifikatCode'=>$sertifikatCodeGet,
                    'sertifikatName'=>$sertifikatNameGet,
                    'needManualInput'=>$needManualInput
                ];

                array_push($arrayDatanya, $arrayDatanya2);
            }

            echo $this->showJson(array('Content-Type: application/json; charset=UTF-8'), array('statusCode'=>200, 'status'=>true, 'message'=>'Berhasil menampilkan data list sertifikat', 'data'=>$arrayDatanya), 200, 'application/json; charset=UTF-8');
